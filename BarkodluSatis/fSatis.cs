    ...
    public partial class FSatis : Form
    {
        ...
        private void UrunGetirListeye(Urun urun, string barkod, double miktar)
        {
            ...
            if (!eklenmismi)
            {
                ...
                gridSatisListesi.Rows[satirsayisi].Cells["UrunAdi"].Value = urun.UrunAdı;
        ...
        private void bOdenen_Click(object sender, EventArgs e)
        {
            if (tNumarator.Text != "")
            {
                ...
                tOdenen.Text = Islemler.DoubleYap(tNumarator.Text).ToString();
                tNumarator.Clear();
                tBarkod.Focus();
        ...
        public void SatisYap(string odemesekli)
        {
            ...
            if (satirsayisi > 0)
            {
                ...
                for (int i = 0; i < satirsayisi; i++)
                {
                    ...
                    db.Satis.Add(satis);
                    db.SaveChanges();
                    alisfiyattoplam += Islemler.DoubleYap(gridSatisListesi.Rows[i].Cells["Fiyat"].Value.ToString());
